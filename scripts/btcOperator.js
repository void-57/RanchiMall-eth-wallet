(function (EXPORTS) {
  ...
  const APIs = (btcOperator.APIs = [
    {
      ...
      name: "Blockcypher",
      balance({ addr }) {
        return fetch_api(`addrs/${addr}/balance`, { url: this.url }).then(
          (result) => util.Sat_to_BTC(result.balance)
        );
      },
      async block({ id }) {
        try {
      ...
      async broadcast({ rawTxHex, url }) {
        try {
          const result = await post(`${url || this.url}pushtx`, {
            tx: rawTxHex,
          });
          return result.hash;
        } catch (e) {
    ...
    },
    {
      ...
      balance({ addr, url }) {
        return fetch_api(`address/${addr}/utxo`, { url: url || this.url }).then(
          (result) => {
      ...
      tx({ txid, url }) {
        return fetch_api(`tx/${txid}`, { url: url || this.url }).then(
          (result) => formatTx(result)
        );
      },
      txHex({ txid, url }) {
        return fetch_api(`tx/${txid}/hex`, {
          url: url || this.url,
          asText: true,
        });
      },
      txs({ addr, url, ...args }) {
        let queryParams = Object.entries(args)
          .map(([key, value]) => `${key}=${value}`)
          .join("&");
        if (queryParams) queryParams = "?" + queryParams;
        return fetch_api(`address/${addr}/txs${queryParams}`, {
          url: url || this.url,
        });
      },
      async block({ id, url }) {
        ...
        try {
          ...
          if (!/^[0-9a-f]{64}$/i.test(id))
            blockHash = await fetch_api(`block-height/${id}`, {
              url: url || this.url,
              asText: true,
            });
          const block = await fetch_api(`block/${blockHash}`, {
            url: url || this.url,
          });
          return formatBlock(block);
        } catch (e) {
    ...
    },
    {
      ...
      name: "Mempool",
      balance({ addr }) {
        return fetch_api(`address/${addr}`, { url: this.url }).then((result) =>
          util.Sat_to_BTC(
      ...
      tx({ txid }) {
        return fetch_api(`tx/${txid}`, { url: this.url }).then((result) =>
          formatTx(result)
        );
      },
      txHex({ txid }) {
    ...
    },
    {
      ...
      name: "Blockchain",
      balance({ addr }) {
        return fetch_api(`q/addressbalance/${addr}`, { url: this.url }).then(
          (result) => util.Sat_to_BTC(result)
        );
      },
      unspent({ addr, allowUnconfirmedUtxos = false }) {
        return fetch_api(`unspent?active=${addr}`, { url: this.url }).then(
          (result) => formatUtxos(result.unspent_outputs, allowUnconfirmedUtxos)
        );
      },
      tx({ txid }) {
        return fetch_api(`rawtx/${txid}`, { url: this.url }).then((result) =>
          formatTx(result)
        );
      },
      txHex({ txid }) {
        return fetch_api(`rawtx/${txid}?format=hex`, {
          url: this.url,
          asText: true,
        });
      },
      txs({ addr, ...args }) {
        let queryParams = Object.entries(args)
          ...
          .join("&");
        if (queryParams) queryParams = "?" + queryParams;
        return fetch_api(`rawaddr/${addr}${queryParams}`, {
          url: this.url,
      ...
      async block({ id }) {
        try {
          ...
          if (/^[0-9a-f]{64}$/i.test(id))
            ...
            const result = await fetch_api(`block-height/${id}?format=json`, {
              url: this.url,
            });
  ...
  const formatBlock = (btcOperator.util.format.block = async (block) => {
    try {
      ...
      if (prev_block) details.prev_block = prev_block;
      if (next_block) details.next_block = next_block[0];
      return details;
    } catch (e) {
      throw e;
    }
  });
  const formatUtxos = (btcOperator.util.format.utxos = async (
    utxos,
    ...
    try {
      let {
        ...
        status: {
          ...
          block_hash: statusBlockHash,
          block_time,
        } = {},
      } = tx;
      if (
        ((block_height || statusBlockHeight) && confirmations === undefined) ||
        confirmations === null
      ) {
      ...
      return {
        ...
        time:
          time * 1000 ||
          new Date(confirmed || received).getTime() ||
          block_time * 1000 ||
          Date.now(),
  ...
  const multiApi = (btcOperator.multiApi = async (
    fnName,
    { index = 0, ...args } = {}
  ) => {
    try {
      ...
      while (index < APIs.length) {
        if (
          !APIs[index][fnName] ||
          (APIs[index].coolDownTime &&
            APIs[index].coolDownTime > new Date().getTime())
        ) {
          index += 1;
          continue;
        }
      ...
      if (isTor && !triedOnion) {
        ...
        while (index < APIs.length) {
          ...
          return await multiApi(fnName, {
            index: index + 1,
            ...args,
            url: APIs[index].onionUrl,
          });
        }
      }
  ...
  function parseTx(tx, addressOfTx) {
    const {
      ...
      received,
      confirmed,
      status: { block_height: statusBlockHeight, block_time } = {},
    } = tx;
    let parsedTx = {
      txid: hash || txid,
      time:
    ...
    (inputs || vin).forEach((i) => {
      ...
      const value =
        i.prev_out?.value || i.output_value || i.value || i.prevout.value;
      if (address in parsedTx.tx_senders) parsedTx.tx_senders[address] += value;
    ...
    // tx fee
    parsedTx.tx_fee = util.Sat_to_BTC(
      fee || fees || parsedTx.tx_input_value - parsedTx.tx_output_value
    );
    //detect tx type (in, out, self)
    if (
      Object.keys(parsedTx.tx_receivers).length === 1 &&
      Object.keys(parsedTx.tx_senders).length === 1 &&
      Object.keys(parsedTx.tx_senders)[0] ===
        Object.keys(parsedTx.tx_receivers)[0]
    ) {
      parsedTx.type = "self";
      parsedTx.amount = parsedTx.tx_receivers[addressOfTx];
      parsedTx.address = addressOfTx;
    } else if (
      addressOfTx in parsedTx.tx_senders &&
      Object.keys(parsedTx.tx_receivers).some((addr) => addr !== addressOfTx)
    ) {
      parsedTx.type = "out";
      parsedTx.receiver = Object.keys(parsedTx.tx_receivers).filter(
        (addr) => addr != addressOfTx
      );
      parsedTx.amount =
        parsedTx.receiver.reduce(
          (t, addr) => t + parsedTx.tx_receivers[addr],
          0
        ) + parsedTx.tx_fee;
    } else {
      parsedTx.type = "in";
      parsedTx.sender = Object.keys(parsedTx.tx_senders).filter(
        (addr) => addr != addressOfTx
      );
      parsedTx.amount = parsedTx.tx_receivers[addressOfTx];
    }
  ...
  const fetch_api = (btcOperator.fetch = function (
    api,
    { asText = false, url = "https://blockchain.info/" } = {}
  ) {
    return new Promise((resolve, reject) => {
  ...
  const broadcastTx = (btcOperator.broadcastTx = (rawTxHex) =>
    ...
    pubkey: {
      value: (key) =>
        key.length >= 66
          ? key
          : key.length == 64
          ? coinjs.newPubkey(key)
          : coinjs.wif2pubkey(key).pubkey,
    },
    address: {
      value: (key, prefix = undefined) =>
        coinjs.pubkey2address(btcOperator.pubkey(key), prefix),
    },
    segwitAddress: {
  ...
  const validateAddress = (btcOperator.validateAddress = function (addr) {
    ...
    if (
      ["standard", "multisig", "bech32", "multisigBech32", "bech32m"].includes(
        type
      )
    )
      return type;
  ...
  btcOperator.convert.wif = function (
    source_wif,
    target_version = coinjs.priv
  ) {
    let keyHex = util.decodeLegacy(source_wif).hex;
    if (!keyHex || keyHex.length < 66 || !/01$/.test(keyHex)) return null;
    else return util.encodeLegacy(keyHex, target_version);
  };

  btcOperator.convert.legacy2legacy = function (
    source_addr,
    target_version = coinjs.pub
  ) {
    let rawHex = util.decodeLegacy(source_addr).hex;
    if (!rawHex) return null;
    else return util.encodeLegacy(rawHex, target_version);
  };

  btcOperator.convert.legacy2bech = function (
    source_addr,
    target_version = coinjs.bech32.version,
    target_hrp = coinjs.bech32.hrp
  ) {
    let rawHex = util.decodeLegacy(source_addr).hex;
    if (!rawHex) return null;
    else return util.encodeBech32(rawHex, target_version, target_hrp);
  };

  btcOperator.convert.bech2bech = function (
    source_addr,
    target_version = coinjs.bech32.version,
    target_hrp = coinjs.bech32.hrp
  ) {
  ...
  btcOperator.convert.bech2legacy = function (
    source_addr,
    target_version = coinjs.pub
  ) {
    let rawHex = util.decodeBech32(source_addr).hex;
  ...
  btcOperator.convert.multisig2multisig = function (
    source_addr,
    target_version = coinjs.multisig
  ) {
    let rawHex = util.decodeLegacy(source_addr).hex;
    if (!rawHex) return null;
    else return util.encodeLegacy(rawHex, target_version);
  ...
  btcOperator.convert.bech2multisig = function (
    source_addr,
    target_version = coinjs.multisig
  ) {
    let rawHex = util.decodeBech32(source_addr).hex;
    if (!rawHex) return null;
      ...
      rawHex = Crypto.util.bytesToHex(
        ripemd160(Crypto.util.hexToBytes(rawHex), { asBytes: true })
      );
  ...
  //BTC blockchain APIs
  btcOperator.getBalance = (addr) =>
    new Promise((resolve, reject) => {
      if (!validateAddress(addr)) return reject("Invalid address");
  ...
  function _redeemScript(addr, key) {
    ...
    switch (decode.type) {
      ...
      case "multisig":
        return key
          ? coinjs.segwitAddress(btcOperator.pubkey(key)).redeemscript
          : null;
      case "bech32":
  ...
  function validateTxParameters(parameters) {
    ...
    if (parameters.senders) {
      ...
      parameters.senders.forEach((id) =>
        !validateAddress(id) ? invalids.push(id) : null
      );
      if (invalids.length) throw "Invalid senders:" + invalids;
    }
    if (parameters.privkeys) {
      ...
      parameters.senders.forEach((id, i) => {
        ...
        if (!verifyKey(id, key))
          ...
          invalids.push(id);
        if (key.length === 64)
          //convert Hex to WIF if needed
          parameters.privkeys[i] = coinjs.privkey2wif(key);
      });
      if (invalids.length) throw "Invalid private key for address:" + invalids;
    }
    //receiver-ids (and change-id)
    if (!Array.isArray(parameters.receivers))
      parameters.receivers = [parameters.receivers];
    parameters.receivers.forEach((id) =>
      !validateAddress(id) ? invalids.push(id) : null
    );
    if (invalids.length) throw "Invalid receivers:" + invalids;
    if (
      parameters.change_address &&
      !validateAddress(parameters.change_address)
    )
      throw "Invalid change_address:" + parameters.change_address;
    //fee and amounts
    if (
      (typeof parameters.fee !== "number" || parameters.fee <= 0) &&
      parameters.fee !== null
    )
      //fee = null (auto calc)
    ...
    parameters.amounts.forEach((a) =>
      typeof a !== "number" || a <= 0 ? invalids.push(a) : null
    );
    if (invalids.length) throw "Invalid amounts:" + invalids;
    //return
    return parameters;
  }
  ...
  const createTransaction = (btcOperator.createTransaction = ({
    ...
    return new Promise((resolve, reject) => {
      let total_amount = parseFloat(
        amounts.reduce((t, a) => t + a, 0).toFixed(8)
      );
      ...
      addInputs(
        ...
        .then((result) => {
          if (result.change_amount > 0 && result.change_amount > result.fee)
            //add change amount if any (ignore dust change)
            tx.outs[tx.outs.length - 1].value = util.BTC_to_Sat(
              result.change_amount
            ); //values are in satoshi
          if (fee_from_receiver) {
            //deduce fee from receivers if fee_from_receiver
          ...
          result.output_amount =
            total_amount - (fee_from_receiver ? result.fee : 0);
          result.total_size = BASE_TX_SIZE + output_size + result.input_size;
          result.transaction = tx;
          if (
            sendingTx &&
            result.hasOwnProperty("hasInsufficientBalance") &&
            result.hasInsufficientBalance
          )
            reject({
    ...
    return new Promise((resolve, reject) => {
      if (fee !== null) {
        addUTXOs(
          ...
          senders,
          redeemScripts,
          fee_from_receiver ? total_amount : total_amount + fee,
          false,
          { allowUnconfirmedUtxos }
        )
          .then((result) => {
            ...
            (fee_from_receiver
              ...
              .then((result) => {
                result.fee = parseFloat(
                  (net_fee + result.input_size * fee_rate).toFixed(8)
                );
  ...
  function addUTXOs(
    ...
    required_amount,
    fee_rate,
    rec_args = { allowUnconfirmedUtxos: false }
  ) {
    return new Promise((resolve, reject) => {
      ...
      multiApi("unspent", {
        addr,
        allowUnconfirmedUtxos: rec_args.allowUnconfirmedUtxos,
      })
        .then((utxos) => {
          ...
          for (let i = 0; i < utxos.length && required_amount > 0; i++) {
            ...
            if (!rs || !rs.length)
              ...
              (rs.match(/^00/) && rs.length == 44) ||
              (rs.length == 40 && rs.match(/^[a-f0-9]+$/gi)) ||
              addr_type === "multisigBech32"
            ) {
            ...
            } //redeemScript for multisig (segwit)
            else script = rs;
            tx.addinput(
              utxos[i].tx_hash_big_endian,
              utxos[i].tx_output_n,
              script,
              0xfffffffd /*sequence*/
            ); //0xfffffffd for Replace-by-fee
            //update track values
            ...
            if (fee_rate)
              //automatic fee calculation (dynamic)
  ...
  function tx_fetch_for_editing(tx) {
    return new Promise((resolve, reject) => {
      if (typeof tx == "string" && /^[0-9a-f]{64}$/i.test(tx)) {
        //tx is txid
        getTx
          .hex(tx)
  ...
  const extractLastHexStrings = (btcOperator.extractLastHexStrings = function (
    arr
  ) {
    const result = [];
  ...
  btcOperator.editFee = function (
    ...
    return new Promise((resolve, reject) => {
      if (!Array.isArray(private_keys)) private_keys = [private_keys];
      tx_fetch_for_editing(tx_hex)
        .then((tx) => {
          parseTransaction(tx)
            .then((tx_parsed) => {
              ...
              if (change_only === true)
                //allow only change values (ie, sender address) to be edited to inc fee
                tx_parsed.inputs.forEach((inp) =>
                  edit_output_address.add(inp.address)
                );
              else if (change_only === false)
                //allow all output values to be edited
                tx_parsed.outputs.forEach((out) =>
                  edit_output_address.add(out.address)
                );
              else if (typeof change_only == "string")
                // allow only given receiver id output to be edited
                edit_output_address.add(change_only);
              else if (Array.isArray(change_only))
                //allow only given set of receiver id outputs to be edited
              ...
              if (inc_fee < MIN_FEE_UPDATE)
                return reject(
                  `Insufficient additional fee. Minimum increment: ${MIN_FEE_UPDATE}`
                );
              for (
                let i = tx.outs.length - 1;
                i >= 0 && inc_fee > 0;
                i-- //reduce in reverse order
              )
                if (edit_output_address.has(tx_parsed.outputs[i].address)) {
                  let current_value = tx.outs[i].value;
                  if (current_value instanceof BigInteger)
                    //convert BigInteger class to inv value
              ...
              if (inc_fee > 0) {
                ...
                return reject(
                  `Insufficient output values to increase fee. Maximum fee possible: ${util.Sat_to_BTC(
                    max_possible_fee
                  )}`
                );
              }
              ...
              for (let i in tx.ins) {
                ...
                if (!privKey) return reject(`Private key missing for ${addr}`);
                //find redeemScript (if any)
                const rs = _redeemScript(addr, privKey);
                rs === false
                  ? wif_keys.unshift(privKey)
                  : wif_keys.push(privKey); //sorting private-keys (wif)
                ...
                if (!rs || !rs.length) {
                  ...
                  (rs.match(/^00/) && rs.length == 44) ||
                  (rs.length == 40 && rs.match(/^[a-f0-9]+$/gi)) ||
                  addr_decode.type === "multisigBech32"
                ) {
                  //redeemScript for segwit/bech32 and multisig (bech32)
                  let s = coinjs.script();
                  s.writeBytes(Crypto.util.hexToBytes(rs));
  ...
  btcOperator.editFee_corewallet = function (
    tx_hex,
    new_fee,
    private_keys,
    change_only = true
  ...
  btcOperator.sendTx = function (
    ...
    return new Promise((resolve, reject) => {
      createSignedTx(senders, privkeys, receivers, amounts, fee, options)
        .then((result) => {
  ...
  const createSignedTx = (btcOperator.createSignedTx = function (
    senders,
    privkeys,
    receivers,
    amounts,
    ...
    return new Promise((resolve, reject) => {
      try {
        ({ senders, privkeys, receivers, amounts } = validateTxParameters({
          senders,
          privkeys,
          receivers,
          amounts,
      ...
      for (let i in senders) {
        ...
        rs === false
          ? wif_keys.unshift(privkeys[i])
          : wif_keys.push(privkeys[i]); //sorting private-keys (wif)
      }
      if (redeemScripts.includes(null))
        //TODO: segwit
  ...
  btcOperator.createTx = function (
    ...
    return new Promise((resolve, reject) => {
      try {
        ({ senders, receivers, amounts } = validateTxParameters({
          senders,
          receivers,
          amounts,
          fee,
  ...
  btcOperator.createMultiSigTx = function (
    sender,
    redeemScript,
    receivers,
    amounts,
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ({ receivers, amounts } = validateTxParameters({
          receivers,
          amounts,
          fee,
          change_address: options.change_address,
  ...
  btcOperator.signTx = function (tx, privkeys, sighashtype = 1) {
    tx = deserializeTx(tx);
    if (!Array.isArray(privkeys)) privkeys = [privkeys];
  ...
  const checkSigned = (btcOperator.checkSigned = function (tx, bool = true) {
    ...
    for (let i in tx.ins) {
      ...
      if (s["type"] !== "multisig" && s["type"] !== "multisig_bech32")
        n.push(
          s.signed == "true" || (tx.witness[i] && tx.witness[i].length == 2)
        );
      else {
  ...
  btcOperator.checkIfSameTx = function (tx1, tx2) {
    ...
    if (
      tx1.ins.length !== tx2.ins.length ||
      tx1.outs.length !== tx2.outs.length
    )
      return false;
    //compare inputs
    for (let i = 0; i < tx1.ins.length; i++)
      if (
        tx1.ins[i].outpoint.hash !== tx2.ins[i].outpoint.hash ||
        tx1.ins[i].outpoint.index !== tx2.ins[i].outpoint.index
      )
        return false;
    //compare outputs
    for (let i = 0; i < tx1.outs.length; i++)
      if (
        tx1.outs[i].value !== tx2.outs[i].value ||
        Crypto.util.bytesToHex(tx1.outs[i].script.buffer) !==
          Crypto.util.bytesToHex(tx2.outs[i].script.buffer)
      )
        return false;
    return true;
  };
  ...
  const parseTransaction = (btcOperator.parseTransaction = function (tx) {
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < tx.ins.length; i++)
        promises.push(
          getTxOutput(tx.ins[i].outpoint.hash, tx.ins[i].outpoint.index)
        );
          ...
          result.outputs = tx.outs.map((out) => {
            ...
            switch (out.script.chunks[0]) {
              case 0: //bech32, multisig-bech32
                address = util.encodeBech32(
                  Crypto.util.bytesToHex(out.script.chunks[1]),
                  coinjs.bech32.version,
                  coinjs.bech32.hrp
                );
                break;
              case 169: //segwit, multisig-segwit
                address = util.encodeLegacy(
                  Crypto.util.bytesToHex(out.script.chunks[1]),
                  coinjs.multisig
                );
                break;
              case 118: //legacy
                address = util.encodeLegacy(
                  Crypto.util.bytesToHex(out.script.chunks[2]),
                  coinjs.pub
                );
            }
            return {
  ...
  const getTx = (btcOperator.getTx = (txid) =>
        ...
        resolve({
          ...
          inputs: result.inputs.map((i) =>
            Object({
              ...
              value: util.Sat_to_BTC(i.prev_out.value),
            })
          ),
          total_input_value: util.Sat_to_BTC(
            result.inputs.reduce((a, i) => a + i.prev_out.value, 0)
          ),
          outputs: result.out.map((o) =>
            Object({ address: o.addr, value: util.Sat_to_BTC(o.value) })
          ),
          total_output_value: util.Sat_to_BTC(
            result.out.reduce((a, o) => (a += o.value), 0)
          ),
        });
      } catch (error) {
        reject(error);
  ...
  btcOperator.getAddressData = (address) =>
    new Promise((resolve, reject) => {
      Promise.all([
  ...
  btcOperator.getBlock = (block) =>
    new Promise((resolve, reject) => {
      fetch_api(`rawblock/${block}`)
        .then((result) =>
          resolve({
            ...
            txs: result.tx.map((t) =>
              Object({
                ...
                total_input_value: util.Sat_to_BTC(
                  t.inputs.reduce((a, i) => a + i.prev_out.value, 0)
                ),
                outputs: t.out.map((o) =>
                  Object({ address: o.addr, value: util.Sat_to_BTC(o.value) })
                ),
                total_output_value: util.Sat_to_BTC(
                  t.out.reduce((a, o) => (a += o.value), 0)
                ),
              })
            ),
          })
